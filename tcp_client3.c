...
float str_cli(FILE *fp, int sockfd, long *len)
{
	...
	int BATCH_NUMBER = 0;  //Batches of size 'n'
	ci = 0;
  ...
  /*** the whole file is loaded in the buffer. ***/
	...
	while(ci<= lsize)
	{
		//sending file in batches
		for (int i = 0; i < BATCH_SIZE && ci <= lsize; i++) {
			...
			// printf("Client sent %d bytes to Server, as part of Packet %d of Batch %d\n", n, i, BATCH_NUMBER);
